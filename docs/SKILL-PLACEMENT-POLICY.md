...
| Evolved | `~/.qwen/homunculus/evolved/skills/` (global) or `projects/<hash>/evolved/skills/` (per-project) | No | Inherits from instinct source |
...
- No provenance file. Use `origin` in SKILL.md frontmatter (EQC, community) for attribution.
...
`scripts/skills-health.js`, `scripts/lib/skill-evolution/health.js`, session hooks: they probe `~/.qwen/skills/learned` and `~/.qwen/skills/imported`. Missing directories are treated as empty; no errors.

## Publishable vs Local-Only

| Publishable | Local-Only |
|-------------|------------|
| `skills/*` (curated) | `~/.qwen/skills/learned/*` |
| | `~/.qwen/skills/imported/*` |
| | `~/.qwen/homunculus/**/evolved/**` |
